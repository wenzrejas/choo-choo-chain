...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  // ── Energy ────────────────────────────────────────────────────────────
  startBoost: () => { if (get().energy > 0) set({ isBoosting: true }) },
  ...
  // ── Shield ────────────────────────────────────────────────────────────
  activateShield: () => set({ shieldActive: true, shieldTimeRemaining: SHIELD_DURATION }),
  ...
  collectWagon: (wagonId, wagonType) => {
    ...
    set({
      score:           score + points,
      wagonsCollected: { ...wagonsCollected, [wagonType]: wagonsCollected[wagonType] + 1 },
  ...
  spawnPowerup:  (powerup) => set((s) => ({ powerups:  [...s.powerups,  powerup] })),

  removeWagon:    (id) => set((s) => ({ wagons:    s.wagons.filter((x)    => x.id !== id) })),
  removeObstacle: (id) => set((s) => ({ obstacles: s.obstacles.filter((x) => x.id !== id) })),
  removePowerup:  (id) => set((s) => ({ powerups:  s.powerups.filter((x)  => x.id !== id) })),

  destroyObstacle: (id) => set((s) => ({
    obstacles: s.obstacles.map((o) => o.id === id ? { ...o, destroyed: true } : o),
  })),
