...
// ─── TreeInstances ────────────────────────────────────────────────────────────
// Renders all non-destroyed tree obstacles as instanced draw calls — one per
// mesh part found in tree.glb. Matrices are rebuilt only when the obstacles
// array reference changes (Zustand emits a new reference on every mutation).
export function TreeInstances(): null {
  const { scene: threeScene }   = useThree()
  const { scene: modelScene }   = useGLTF(TREE_MODEL_PATH)
  ...
  useEffect(() => {
    const meshes = parts.map(({ geometry, material }) => {
      const m = new THREE.InstancedMesh(geometry, material, MAX_TREES)
      m.count         = 0
      m.castShadow    = true
