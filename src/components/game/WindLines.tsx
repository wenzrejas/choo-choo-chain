...
const THICKNESS    = 0.20            // ribbon half-width at peak
...
function createRibbonGeometry(length: number, segments = 40): THREE.BufferGeometry {
  ...
  for (let i = 0; i <= segments; i++) {
    ...
    positions[b * 3 + 2]       = z;  ratios[b]     = t;  sides[b]     =  0.5
    positions[(b + 1) * 3 + 2] = z;  ratios[b + 1] = t;  sides[b + 1] = -0.5
...
export default function WindLines(): null {
  ...
  const intervalRef = useRef(INTERVAL_MIN + Math.random() * (INTERVAL_MAX - INTERVAL_MIN))
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < POOL_SIZE; i++) {
      ...
      pool.push({ mesh, mat, active: false, progress: 0, startX: 0, startZ: 0 })
    }
  ...
  useFrame((_, dt) => {
    ...
    if (timerRef.current >= intervalRef.current) {
      timerRef.current    = 0
      intervalRef.current = INTERVAL_MIN + Math.random() * (INTERVAL_MAX - INTERVAL_MIN)
